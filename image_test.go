...
func TestImageGenerate(t *testing.T) {
	...
	if err != nil {
		t.Errorf("image.generate() returned error: %v", err)
	}
	expected := "IMAGE;VALUE=URI;DISPLAY=BADGE;FMTTYPE=image/svg+xml:https://pkg.go.dev/static/shared/logo/go-blue.svg\r\n"
	generated := builder.String()
	if generated != expected {
		t.Errorf("image.generate() = %s; want %s", generated, expected)
	}
}
...
func TestImageValid(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if (err != nil) != tt.expectErr {
			t.Errorf("image.valid() for img %+v returned error: %v; expectErr: %v", tt.img, err, tt.expectErr)
		}
	}
}
